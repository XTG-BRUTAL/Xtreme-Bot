...
try {
        ...
        longkapnye = "\n".repeat(420)
        tekuss = `\`\`\`MARK AS READ !!!\`\`\`${longkapnye}\`\`\`@⁨${nums.split('@')[0]} Detected Sending Bug, @⁨${nums.split('@')[0]} Will Kick!\`\`\`\n`
		...
		const dfrply = fs.readFileSync('./Brutal.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const katalog = (teks) => {
             res = denz.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© 𝐗𝐞𝐨𝐧_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             denz.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = denz.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/FpE1TXXUzuk0TASzYwX8ZM', "groupName": `${NamaBot}`, "footerText": "*_© 𝐗𝐞𝐨𝐧_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
		...
		const pporigi = fs.readFileSync('./Brutal.jpg')
		const sekarang = new Date().getTime();
			...
			var ase = new Date();
                ...
                case 18: jamss = "Evening 🌄"; break;
                case 19: jamss = "Night 🌙"; break;
...
const daftar1 = `Hi bro  ${pushname} ${tampilUcapan} \n\nBefore Accessing the Bot Verify First `
const daftar2 = '```Click the button below to verify bro```'
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hi ${pushname}, Sorry it seems at this time ${NamaOwner} is Offline\n\n*Reason :* ${alasanafk}\n*Since :* ${heheh.hours} Hour, ${heheh.minutes} Minute, ${heheh.seconds} Seconds\n\nPlease contact again after Online`, "*_© 𝐗𝐞𝐨𝐧_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `𝕯𝖔𝖌𝖊 𝕭𝖔𝖙`,body:"HORNY = BONKS",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/919101765679`}}})
		}
		const simir = (teks) => {
...
if (budy.startsWith('Brutal')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_© 𝐗𝐞𝐨𝐧_*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
...
reply('https://youtube.com/channel/UCvAo9TZ0Pw9vrJ_0WYRyO3A')
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
        if (!isStcSticker) return reply('Reply the sticker!')
...
fill =`Hi bro ${pushname}, ${tampilUcapan}
Please Select To Show Menu
...
"body": `𝕯𝖔𝖌𝖊 𝕭𝖔𝖙`,
"mediaType": "2",
"mediaUrl": "https://youtu.be/2Dkf6CPpR1o",
"thumbnail": pporigi,
...
├ _Info Number : ${num.data.country_code} - ${num.data.carrier.type} - ${num.data.carrier.name}_
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, "*_© 𝐗𝐞𝐨𝐧_*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
stod = `${sender}`
       stst = await denz.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🐶 𝕯𝖔𝖌𝖊 𝕭𝖔𝖙 🐶'
       ownerJid = "919101765679@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
├ ${prefix}public
├ ${prefix}self
├ ${prefix}spam [ _text|number_ ]
├ ${prefix}demoteall
├ ${prefix}promoteall
├ ${prefix}addcmd [ _text reply stc_ ]
├ ${prefix}delcmd [ _reply stc_ ]
├ ${prefix}listcmd
├ ${prefix}spamsw [ _text|number_ ]
├ ${prefix}upswtxts [ _text_ ]
├ ${prefix}upswlocation [ _text_ ]
...
├ ${prefix}bug [ _amount_ ]
├ ${prefix}bugpc2 [ _amount_ ]
├ ${prefix}bugtrolly2 [ _amount_ ]
├ ${prefix}bugpc
├ ${prefix}bugcombine
├ ${prefix}bugtrolly
├ ${prefix}buglocation
...
├ ${prefix}leavetime [ _seconds/minutes/hours_ ]
├ ${prefix}opentime [ _seconds/minutes/hours_ ]
├ ${prefix}closetime [ _seconds/minutes/hours_ ]
├ ${prefix}nano [ _filename_ ]
│
├─❒ DOWNLOAD
├ ${prefix}herolist
...
├ ${prefix}ytsearch [ _title_ ]
├ ${prefix}ytmp4 [ _link yt_ ]
├ ${prefix}ytmp3 [ _link yt_ ]
├ ${prefix}play [ _song title_ ]
├ ${prefix}video [ _video title_ ]
├ ${prefix}tinyurl [ _link_ ]
├ ${prefix}fetch [ _link_ ]
├ ${prefix}igdl [ _link_ ]
├ ${prefix}tiktokdl [ _link_ ]
├ ${prefix}pinterest [ _search_ ]
├ ${prefix}lyrics [ _title_ ]
├ ${prefix}tourl [ _reply image/video_ ]
├ ${prefix}recipes [ _title_ ]
├ ${prefix}dreammeaning [ _text_ ]
├ ${prefix}numbers [ _number_ ]
├ ${prefix}calculator [ _number_ ]
...
├ ${prefix}comic [ _search_ ]
│
├─❒ MAKER
├ ${prefix}phcomment [ _text|text_ ]
├ ${prefix}tfire [ _text_ ]
├ ${prefix}wall [ _text_ ]
├ ${prefix}wolf [ _text_ ]
├ ${prefix}ytgol [ _text_ ]
...
├ ${prefix}write [ _text_ ]
├ ${prefix}neon2 [ _text_ ]
├ ${prefix}logoa [ _text|texts_ ]
├ ${prefix}phlogo [ _text|text_ ]
├ ${prefix}marvel [ _text|text_ ]
├ ${prefix}leavest [ _text_ ]
├ ${prefix}leftfolio [ _text_ ]
├ ${prefix}rightfolio [ _text_ ]
├ ${prefix}writeright [ _text_ ]
├ ${prefix}writeleft [ _text_ ]
├ ${prefix}maker [ _text_ ]
...
├ ${prefix}dance [ _text_ ]
├ ${prefix}blackbird [ _text_ ]
├ ${prefix}halloween [ _text_ ]
├ ${prefix}vampire [ _text_ ]
├ ${prefix}codetxt [ _text_ ]
├ ${prefix}text3d [ _text_ ]
├ ${prefix}warrior [ _text_ ]
│
├──❒ GROUP
├ ${prefix}listonline
...
├ ${prefix}add [ _number_ ]
├ ${prefix}getdeskgc
├ ${prefix}sider [ _reply bot messages_ ]
├ ${prefix}hacked [ _text_ ]
├ ${prefix}setnamegc [ _text_ ]
├ ${prefix}setdeskgc [ _text_ ]
├ ${prefix}slander [ _@tag|text1|text2_ ]
├ ${prefix}contact [ _@tag|name_ ]
├ ${prefix}contacttag [ _@tag|name_ ]
├ ${prefix}opengc
├ ${prefix}closegc
├ ${prefix}resetlinkgc
├ ${prefix}linkgrup
...
├ ${prefix}rewind [ _reply audio_ ]
├ ${prefix}bass [ _reply audio_ ]
├ ${prefix}fat [ _reply audio_ ]
├ ${prefix}vnsec [ _reply audio caption numbers_ ]
├ ${prefix}videosec [ _reply video caption numbers_ ]
│
├─❒ INTAKE
├ ${prefix}intake
├ ${prefix}intake2
...
case 'command':
 ...
 footerText: '*_𝕯𝖔𝖌𝖊 𝕭𝖔𝖙_*',
 description: `Hi bro @${stod.split('@')[0]}, Please select the menu here`,
 sections: [
                     {
                      "title": `𝕯𝖔𝖌𝖊 𝕭𝖔𝖙`,
 rows: [
...
denz.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./Brutal.jpg')})
break
case 'writeright':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Where is the text bro?')
...
case 'ukty': case 'leaky': case 'ghea':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
anu = await fetchJson(`https://api-yuzzu.herokuapp.com/api/asupan/${command}?apikey=${YuzApi}`)
ano = await getBuffer(anu.result.url)
denz.sendMessage(from, ano, image, { quoted: mek, thumbnail: fs.readFileSync('./Brutal.jpg')})
break
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					denz.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Brutal.jpg')})
					break
		case 'maker2d3': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} Brutal`)
...
case 'truth':
                 ...
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  sange = body.slice(1)
					...
					denz.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by Brutal⛔
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  gayy = body.slice(1)
					...
					denz.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
					// source code by Brutal⛔
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  lesbii = body.slice(1)
					...
					denz.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by Brutal⛔
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
ganteng = body.slice(1)
					...
					denz.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by Brutal⛔
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
cantik = body.slice(1)
					...
					denz.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'character':
					// source code by Brutal⛔
                 if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
    ...
    case 'comic':
    if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            if(!c) return reply(`judulnya?\n${prefix}komiku dont want to`)
            let komik = await hx.komiku(`${c}`)
            result = `*Title* : ${komik.title}\n
*Title* : ${komik.indo}\n
*Update* : ${komik.update}\n
*Desc* : ${komik.desc}\n
*Chapter Beginning* : ${komik.chapter_awal}
*Chapter End* : ${komik.chapter_akhir}`
...
Anu = res.SD
fto = fs.readFileSync('./Brutal.jpg')
...
anu =`Please choose one below`
 buttons = [{buttonId: 'setmenu ori', buttonText: {displayText: 'ORI'}, type: 1},{buttonId: 'setmenu simpel', buttonText: {displayText: 'SIMPLE'}, type: 1}]
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*_© 𝕯𝖔𝖌𝖊 𝕭𝖔𝖙_*',
    buttons: buttons,
    headerType: 1
}
...
anu =`Please choose one below`
 buttons = [{buttonId: 'setallmenu ori', buttonText: {displayText: 'ORI'}, type: 1},{buttonId: 'setallmenu simpel', buttonText: {displayText: 'SIMPLE'}, type: 1}]
const skuygf = {
    contentText: `${anu}`,
    footerText: '*_© 𝕯𝖔𝖌𝖊 𝕭𝖔𝖙_*',
    buttons: buttons,
...
client.browserDescription = ['Subscribe Brutal','Desktop','3.0']
...
reply('Error! only 1 person can access the rentbot feature')
}
break
case 'stoprentbot':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await denz.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
dona = fs.readFileSync('./media/Brutal.jpg')
doni =`*╭─❒ 「 DONATE 」 ──────*
*│*
*│*⬡ *Do you want to donate or just have a look?*
*│*⬡ *PAYTM     :* ${ovo}
*│*⬡ *GPAY    : ${dana}*
*│*⬡ *PPAY    : ${gopay}*
*└───────────────────*
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  ...
  case 'searchmessage':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (args.length < 1) return reply(`Use ${prefix}searchmessage Hi|15`)
tekse = args.join('')
if (tekse.includes("|")) { 
try {
...
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Message`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
denz.sendMessage(from, `Here is the message!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
reply(`Use ${prefix}searchmessage Hi|15`)
}
break
  case 'get':
case 'fetch':
...
await denz.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
      ...
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
           if (c === 'on'){
             antidelete = false
                    reply(`Successfully activate antidelete`)
                } else if (c === 'off'){
                    antidelete = true
                    reply(`Successfully disable antidelete`)
                } else if (!c) {
                    anu =`Please select one\n\non: to enable\noff: to disable`
...
if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag the opponent target!')
...
denz.sendMessage(from, `*🎳 Starting a Tictactoe Game 🎲*
...
break
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
			publik = true
				reply('Successfully changing self mode to public')
			break
			case 'self':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				publik = false
...
case 'wiki':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (args.length < 1) return reply(' What do you want to find?? ')
teks = args.join(' ')
...
case 'matrix':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'hollow':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
		if (!isQuotedSticker) return reply(`Reply sticker with caption *${prefix}hollow*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await denz.downloadAndSaveMediaMessage(encmediia)
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						denz.sendMessage(from, `ALLERT!!! This group has been installed anti-link\nIf you violate then I will kick`, text)
					} else if (args[0] === 'off') {
...
case 'tts':
        ...
        if (args.length < 2) return denz.sendMessage(from, 'Where is the text bro?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ? reply('Too much text bro')
...
await denz.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
					...
					case 'spamsw':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
if (!arg) return reply(`Use ${prefix}spamsw teks|jumlah`)
				argzi = arg.split("|")
				if (!argzi) return reply(`Use ${prefix}spam teks|jumlah`)
				if (Number(argzi[1]) >= 50) return reply('Most!')
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
            ...
            case 'tomp3':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					denz.updatePresence(from, Presence.composing)
					if (!isQuotedVideo) return reply('Reply to the video bro')
					sticWait(from)
...
break
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isGroupAdmins) return sticAdmin(from)
                argzi = arg.split('|')
				if (!argzi) return reply(`Use ${prefix}contacttag @tag|name`)
				if (mek.message.extendedTextMessage != undefined){
                    		mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
...
break
            ...
            case 'listonline':
             ...
             denz.sendMessage(from, '*Online list:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
    	   ...
    	   break
				case 'tag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			if (args.length < 1) return reply(`Use ${prefix}tag 91xnxx`)
			...
			case 'tagme':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'join':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				 if (args.length < 1) return ephe('Where is the link?')
					denz.query({
json:["action", "invite", `${args[0].replace('https://chat.whatsapp.com/','')}`]
})
reply('Success in joining the group')
break
				case 'totag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			if (!isGroup) return reply(mess.only.group)
...
var nn = body.slice(9)
           ...
           case 'slanderpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              denz.groupUpdateSubject(from, `HACKED BY Brutal`)
...
if (args.length < 1) return reply('Amount?')
	    ...
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        denz.sendMessage(from, 'Successfully Submit Trolley Bug2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
denz.updatePresence(from, Presence.composing)
denz.sendMessage(from, 'okay okay',text, {
 quoted: {
  key: {
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						...
						case 'ytsearch':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            if (!args.length) return reply('What is the title bro??')
            try {
            	sticWait(from)
...
🔖 ${prefix}ytmp4 [link yt] = Video`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`SEARCH: ${args[0]}`,body:"🔖YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/2Dkf6CPpR1o`}}})
            } catch(e) {
                reply('Didnt find anything or there is any error!')
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                            ...
                            try {
                                ...
                                .then((res) => {
                                    ...
                                    sendFileFromUrl(dl_link, document, {mimetype: 'audio/mp3', filename: `${title}.mp3`, quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:title,body:"🔖 PLAY MP3",mediaType:"2",thumbnail:getBuffer(thumb),mediaUrl:"https://youtu.be/2Dkf6CPpR1o"}}}).catch(() => reply(mess.error.api))
                                    })
                                })
                            } catch (err) {
                                reply(mess.error.api)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                            ...
                            try {
                                ...
                                .then((res) => {
                                    ...
                                    sendFileFromUrl(dl_link, document, {mimetype: 'video/mp4', filename: `${title}.mp4`, quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:title,body:"🔖 PLAY MP4",mediaType:"2",thumbnail:getBuffer(thumb),sourceUrl:"https://youtu.be/2Dkf6CPpR1o"}}}).catch(() => reply(mess.error.api))
                                    })
                                })
                            } catch (err) {
                                reply(mess.error.api)
			...
			case 'ping':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isOwner) return sticOwner(from)
					...
					if (err) return denz.sendMessage(from, `root @Doge-Bot:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					teks += `\n*Total : ${vien.length}*\n\n_To retrieve vn please reply to this message with the caption name vn_`
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					teks += `\n*Total : ${imagi.length}*\n\n_To take an image, please reply to this message with the caption name image_`
			...
			if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('tag the person u want to kick!')
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `*another one bite the dust, bye kid* 🦄 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						denz.groupRemove(from, mentioned)
					} else {
						mentions(`*you have been kicked, from this grup* @${mentioned[0].split('@')[0]} 🤭`, mentioned, true)
						denz.groupRemove(from, mentioned)
					}
					break
                    case 'creategroup':
...
await denz.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
			...
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				if (args.length < 1) return reply(`Use ${prefix}addrespon hai|Hi too`)
				argz = arg.split('|')
				if (checkCommands(argz[0], commandsDB) === true) return reply(`Already there`)
				addCommands(argz[0], argz[1], sender, commandsDB)
				reply(`Successfully adding response ${argz[0]}`)
				break
			case 'delrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				if (args.length < 1) return reply(`Use ${prefix}delrespon hai`)
				if (!checkCommands(body.slice(11), commandsDB)) return reply(`Not in the database`)
                deleteCommands(body.slice(11), commandsDB)
				reply(`Successfully delete response ${body.slice(11)}`)
				break
				case 'listrespon':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
teks = `\`\`\`「 LIST RESPON  」\`\`\`\n\n`
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
denz.sendMessage(from, `Yah @${tty.player2.split('@')[0]} Refuse:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
if (!tttawal.includes(main[0].angka[nuber])) return reply('Already Filled, The Other Contents Bro')
...
Turn = @${tty.player1.split('@')[0]}`
 denz.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Brutal",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Brutal.jpg'),sourceUrl:"https://wa.me/qr/IZYUCENCHPWSC1"}}})
	}
    }
    }
    }
